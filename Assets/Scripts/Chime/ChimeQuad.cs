...
public class ChimeQuad
{
    ...
    public ChimeQuad(List<Vector2Int> quadCells, int currentTurn = 0, int initialTurnTimer = 0)
    {
    ...
    // 다른 Quad와 겹치는지 확인
    public bool OverlapsWith(ChimeQuad other)
    {
        foreach (var cell in cells)
        {
    ...
    public bool IsAdjacentTo(ChimeQuad other)
    {
        // 4방향 (상하좌우)
    ...
    public bool CanMergeWith(ChimeQuad other)
    {
        // 두 Quad의 모든 셀을 합침
    ...
    // 다른 Quad와 병합
    public ChimeQuad MergeWith(ChimeQuad other, int currentTurn)
    {
        HashSet<Vector2Int> mergedCells = new HashSet<Vector2Int>(cells);
        foreach (var cell in other.cells)
        {
        ...
        return new ChimeQuad(new List<Vector2Int>(mergedCells), currentTurn, preservedTurnTimer);
    }
}
